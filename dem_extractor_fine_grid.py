...
def process_folder(folder, base_path, step_meters, num_points, rotation_angles, api_key, workers=None, seed=None, use_fua=False, fua_path=None, resume=False):
...
def process_variant(args):
    ...
    if G_var is None:
        ...
        # Check if all nodes are on land (strict check: 100% required)
        # Use sampled nodes for consistency with offset generation
        sample_nodes = sample_graph_nodes(G_var)
        nodes_on_land = all(Point(d['x'], d['y']).within(_LAND_MASK) for _, d in sample_nodes)
...
def main(api_key=None, example_city=None, step_meters=50, num_points=10, 
         rotation_angles=None, workers=None, seed=None, use_fua=False, fua_path=None, resume=False):
    ...
    rotation_angles = [(a % 360) for a in rotation_angles] + [(360 - a) % 360 for a in rotation_angles]

    if example_city:
        process_folder(example_city, base_path, step_meters, num_points, 
                      rotation_angles, api_key, workers, seed, use_fua, fua_path, resume=resume)
    else:
...
if __name__ == '__main__':
    ...
    if args.test:
        ...
        args.seed = 42
        args.use_fua = True
        logger.info("Running in TEST mode with: step=50m, points=3, rotations=[-5,5], workers=3, seed=42, use_fua=True")
    
    main(args.api_key, args.city, args.step_meters, args.num_points,
            args.rotation_angles, args.workers, args.seed, args.use_fua, args.fua_path, resume=args.resume)
