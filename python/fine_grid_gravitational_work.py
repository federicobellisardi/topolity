#!/usr/bin/env python3
"""
Calculate gravitational work for all fine grid graph transformations.

Author: Federico Bellisardi
Date: 2025-11-28

To create the graph:
cd /home/fbellisardi/code/topolity && conda activate geo_flow && runlog -t 12:00 -m 350 -c 8 -j fine_grid_santi python dem_extractor_fine_grid.py --city santiago --step-meters 500 --num-points 5 --rotation-angles -20 -15 -10 -5 5 10 15 20 --workers 8 --seed 42


This script:
1. Loads all graphs from graphs_fine_grid directory
2. Computes total uphill gravitational work for each graph
3. Creates a scatter plot showing work vs transformation parameter
4. Expected: original configuration at the vertex of a parabola

Usage:
    runlog -t 120:00 -m 350 -j fine_work_santi python fine_grid_gravitational_work.py
"""
